        ...
        <div class="space-y-4">
            ...
            <Transition name="slide-down">
                <div v-if="props.emailCheckState === 'valid'" class="space-y-2">
                    ...
                    <div class="relative">
                    <Input id="password" ref="passwordInput" :model-value="props.password" @update:model-value="handlePasswordUpdate"
...
// Watch for email validation success and focus password field
watch(() => props.emailCheckState, async (newState) => {
    if (newState === 'valid') {
        await nextTick()
        // Use optional chaining and $el to ensure we target the input element
        if (passwordInput.value && passwordInput.value.$el) {
            const input = passwordInput.value.$el.querySelector('input') || passwordInput.value.$el
            input.focus()
        }
    }
})
