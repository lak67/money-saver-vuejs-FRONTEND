...
const handleFormSubmit = () => {
    switch (currentStep.value) {
        case 'login':
            if (isEmailFormatValid.value && loginEmailCheckState.value !== 'loading' && !isLoading.value) {
                submitLogin()
            }
            break
        case 'register':
            if (isEmailFormatValid.value && emailCheckState.value !== 'loading' && !isLoading.value) {
                submitRegister()
            }
            break
        case 'income':
            if (isIncomeFormValid.value && !isLoading.value) {
                submitIncome()
            }
            break
        case 'budget-types':
            if (isBudgetTypesFormValid.value && !isLoading.value) {
                submitBudgetTypes()
            }
            break
        case 'budget-amounts':
            if (isBudgetAmountsFormValid.value && !isLoading.value) {
                submitBudgetAmounts()
            }
            break
        case 'confirmation':
            ...
            <form @submit.prevent="handleFormSubmit" class="contents">
                ...
                <DialogFooter class="gap-2">
                    ...
                    <template v-if="currentStep === 'income'">
                        ...
                        <Button type="submit" :disabled="!isIncomeFormValid || isLoading">
                            Next →
                        </Button>
                    </template>
                    ...
                    <template v-if="currentStep === 'budget-types'">
                        ...
                        <Button type="submit" :disabled="!isBudgetTypesFormValid || isLoading">
                            Next →
                        </Button>
                    </template>
                    ...
                    <template v-if="currentStep === 'budget-amounts'">
                        ...
                        <Button type="submit" :disabled="!isBudgetAmountsFormValid || isLoading">
                            Review →
                        </Button>
                    </template>
