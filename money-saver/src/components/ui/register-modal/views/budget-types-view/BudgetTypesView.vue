    ...
    <div class="space-y-6 py-4">
        ...
        <div class="space-y-4">
            ...
            <div class="flex items-center justify-between">
                <p class="text-sm text-muted-foreground">
                    {{ getSelectionSummary(selectedBudgetTypes, availableBudgetTypes) }}
                </p>
        ...
        <div v-if="!searchTerm && showRecommended">
            ...
            <div class="grid grid-cols-1 gap-3">
                <BudgetTypeCard v-for="budgetType in getRecommendedTypes(availableBudgetTypes)" :key="budgetType.id"
        ...
        <div v-if="!searchTerm && showRecommended">
            ...
            <div class="grid grid-cols-1 gap-3">
                <BudgetTypeCard v-for="budgetType in getOtherTypes(availableBudgetTypes)" :key="budgetType.id"
                    :budget-type="budgetType" :is-selected="selectedBudgetTypes.includes(budgetType.id)"
                    @toggle="$emit('toggle', budgetType.id)" />
            </div>
        </div>
        ...
        <div v-if="searchTerm">
            ...
            <div class="grid grid-cols-1 gap-3">
                <BudgetTypeCard v-for="budgetType in filteredBudgetTypes(availableBudgetTypes)" :key="budgetType.id"
                    :budget-type="budgetType" :is-selected="selectedBudgetTypes.includes(budgetType.id)"
                    @toggle="$emit('toggle', budgetType.id)" />
            </div>
            <p v-if="filteredBudgetTypes(availableBudgetTypes).length === 0"
...
const handleSelectRecommended = () => {
    selectRecommended(props.availableBudgetTypes, (id: string) => emit('toggle', id), props.selectedBudgetTypes)
}
